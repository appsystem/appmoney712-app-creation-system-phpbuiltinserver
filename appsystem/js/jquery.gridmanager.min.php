...
/*------------------------------------------ Canvas & Controls ---------------------------------------*/
        ...
        gm.initDefaultButtons = function(){
          ...
          if(gm.options.editableRegionEnabled) {
            gm.options.customControls.global_col.push({callback: gm.addEditableAreaClick, loc: 'top', iconClass: 'fa fa-edit', title: '<?php if($_SESSION[tn]==PRC){echo '添加编辑区';}else if($_SESSION[tn]==EN){echo 'Add Editing Region';}else{echo '添加編輯區';}?>'});
          }
        };
        ...
        gm.createControls = function(){
          gm.log("+ Create Controls");
            ...
            $.each(gm.options.controlButtons, function(i, val){
              ...
              buttons.push("<a title='<?php if($_SESSION[tn]==PRC){echo '添加行';}else if($_SESSION[tn]==EN){echo 'Add Row';}else{echo '添加行';}?> " + _class + "' style='color:white' class='" + gm.options.controlButtonClass + " add" + _class + "'><span class='" + gm.options.controlButtonSpanClass + "'></span> " + _class + "</a>");
      ...
      /**
        ...
        var canvas=gm.$el.find("#" + gm.options.canvasId);
            $.ajax({
              ...
              data: {popuppicda: canvas.html()}
            });
            gm.log("Save Function Called");
			window.location.href = "<?php if($_GET[typ]=='popuppicda'){ echo 'popuppicda';}else if($_GET[typ]=='popuppicsd'){ echo 'popuppicsd';}else if($_GET[typ]=='editorifr'){ echo 'editorifr';}?>.php?pj=<?php echo $pj?>&ap=<?php echo $ap?>&pn=<?php echo $pn?>&typ=<?php if($_GET[typ]=='popuppicda'){ echo 'popuppicda';}else if($_GET[typ]=='popuppicsd'){ echo 'popuppicsd';}else if($_GET[typ]=='editorifr'){ echo 'editorifr';}?>";
        };
...
/*------------------------------------------ Edit Regions ---------------------------------------*/
        ...
        /*
          Callback called when a the new edit area button is clicked
        ...
        gm.addEditableAreaClick = function(container, btn) {
          ...
          .before(elem = $('<div>').addClass(gm.options.gmEditRegion+' '+gm.options.contentDraggableClass)
            .append(gm.options.controlContentElem+'<div class="'+gm.options.gmContentRegion+'"><p><?php if($_GET[typ]=='popuppicda' or $_GET[typ]=='popuppicsd' or $_GET[typ]=='editorifr'){
						echo '<a href="#" class="img ui-btn ui-btn-w ui-btn-inline ui-mini"><span class="pigss-pictures"></span></a><a href="#" class="udtr ui-btn ui-btn-w ui-btn-inline ui-mini"><span class="pigss-article"></span></a>';	
					}else{
						if($_SESSION[tn]==PRC){echo '添加內容';}else if($_SESSION[tn]==EN){echo 'New Content';}else{echo '添加內容';}
					;}?></p></div>')).before(cTagClose).prev().before(cTagOpen);
...
/*------------------------------------------ Useful functions ---------------------------------------*/
    ...
    */
        ...
        // URL to save to
        remoteURL: "<?php if($_GET[typ]=='popuppicda'){
						echo 'popuppicda.php?pj='.htmlspecialchars($_GET[pj]).'&ap='.htmlspecialchars($_GET[ap]).'&pn='.htmlspecialchars($_GET[pn]).'&typ=popuppicda';			
					}else if($_GET[typ]=='popuppicsd'){
						echo 'popuppicsd.php?pj='.htmlspecialchars($_GET[pj]).'&ap='.htmlspecialchars($_GET[ap]).'&pn='.htmlspecialchars($_GET[pn]).'&typ=popuppicsd';			
					}else if($_GET[typ]=='editorifr'){
						echo 'editorifr.php?pj='.htmlspecialchars($_GET[pj]).'&ap='.htmlspecialchars($_GET[ap]).'&pn='.htmlspecialchars($_GET[pn]).'&typ=editorifr';			
					}?>",

        // Custom CSS to load
        cssInclude: "../bootstrap/fonts/style.css",
  ...
  */
		...
		//< button title='Edit Source Code' type='button' class='btn btn-xs btn-primary gm-edit-mode'><span class='fa fa-code'></span></button>
        controlAppend: "<div class='btn-group pull-right'><button title='<?php if($_SESSION[tn]==PRC){echo '预览';}else if($_SESSION[tn]==EN){echo 'Preview';}else{echo '預覽';}?>' type='button' class='btn btn-xs btn-primary gm-preview'><span class='fa fa-search'></span></button>     <div class='dropdown pull-right gm-layout-mode'><button type='button' class='btn btn-xs btn-primary dropdown-toggle' data-toggle='dropdown'><span class='caret'></span></button> <ul class='dropdown-menu' role='menu'><li><a data-width='auto' title='Desktop'><span class='fa fa-desktop'></span> Desktop</a></li><li><a title='Tablet' data-width='768'><span class='fa fa-tablet'></span> Tablet</a></li><li><a title='Phone' data-width='640'><span class='fa fa-mobile-phone'></span> Phone</a></li></ul></div>    <button type='button' class='btn  btn-xs  btn-primary dropdown-toggle' data-toggle='dropdown'><span style='color:pink' class='fa fa-save'></span></button><ul class='dropdown-menu' role='menu'><li><a title='Save'  id='gmsave'  href='#' class='gm-save'><span class='fa fa-save'></span> <?php if($_SESSION[tn]==PRC){echo '储存';}else if($_SESSION[tn]==EN){echo 'Save';}else{echo '儲存';}?></a></li><li><a>&nbsp;</a></li><li><a title='Reset Grid' href='#' class='gm-resetgrid'><span class='fa fa-trash-o'></span> <?php if($_SESSION[tn]==PRC){echo '清除全部';}else if($_SESSION[tn]==EN){echo 'Reset';}else{echo '清除全部';}?></a></li></ul></div>",

        // Controls for content elements
        controlContentElem: '<div class="gm-controls-element"> <a class="gm-move fa fa-arrows" href="#" title="<?php if($_SESSION[tn]==PRC){echo '搬移';}else if($_SESSION[tn]==EN){echo 'Move';}else{echo '搬移';}?>"></a> <a class="gm-delete fa fa-times" href="#" title="<?php if($_SESSION[tn]==PRC){echo '刪除';}else if($_SESSION[tn]==EN){echo 'Delete';}else{echo '刪除';}?>"></a> </div>',
   /*
  ...
  */
        ...
        gmEditRegion: "gm-editing-region",
  ...
  */
        ...
        rowButtonsPrepend: [
                {
                 title:"<?php if($_SESSION[tn]==PRC){echo '搬移';}else if($_SESSION[tn]==EN){echo 'Move';}else{echo '搬移';}?>",
                 element: "a",
                 btnClass: "gm-moveRow pull-left",
                 iconClass: "fa fa-arrows "
              },
                {
                   title:"<?php if($_SESSION[tn]==PRC){echo '添加项';}else if($_SESSION[tn]==EN){echo 'New Column';}else{echo '添加項';}?>",
                   element: "a",
                   btnClass: "gm-addColumn pull-left  ",
                   iconClass: "fa fa-plus"
                },
                 {
                   title:"<?php if($_SESSION[tn]==PRC){echo '行设定';}else if($_SESSION[tn]==EN){echo 'Row Settings';}else{echo '行設定';}?>",
                   element: "a",
        ...
        rowButtonsAppend: [
                {
                 title:"<?php if($_SESSION[tn]==PRC){echo '刪除全部';}else if($_SESSION[tn]==EN){echo 'Remove all';}else{echo '刪除全部';}?>",
                 element: "a",
  ...
  */
       ...
       // Generic desktop size layout class
        ...
        colButtonsPrepend: [
              ...
              {
                   title:"<?php if($_SESSION[tn]==PRC){echo '设定';}else if($_SESSION[tn]==EN){echo 'Column Settings';}else{echo '設定';}?>",
                   element: "a",
                   btnClass: "pull-right gm-colSettings",
                   iconClass: "fa fa-cog"
                },
               {
                 title:"<?php if($_SESSION[tn]==PRC){echo '收窄';}else if($_SESSION[tn]==EN){echo 'Create Column Narrower';}else{echo '收窄';}?>",
                 element: "a",
                 btnClass: "gm-colDecrease pull-left",
                 iconClass: "fa fa-minus"
              },
              {
               title:"<?php if($_SESSION[tn]==PRC){echo '加阔';}else if($_SESSION[tn]==EN){echo 'Create Column Wider';}else{echo '加闊';}?>",
               element: "a",
        ...
        colButtonsAppend: [
                {
                 title:"<?php if($_SESSION[tn]==PRC){echo '移除';}else if($_SESSION[tn]==EN){echo 'Remove Column';}else{echo '移除';}?>",
                 element: "a",
             ...
             //    {
              //    title:"<?php //if($_SESSION[tn]==PRC){echo '加巢形行';}else if($_SESSION[tn]==EN){echo 'Add Nested Row';}else{echo '加巢形行';}?>",//colButtonsAppend:
               //   element: "a",
