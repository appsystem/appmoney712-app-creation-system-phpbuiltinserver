...
<div data-role="page" data-theme="f" class="page" style="background-color:white;color:black">
	...
	<a href="menudesign.php?pj=<?php echo htmlspecialchars($roww[pjnbr])?>&ap=<?php echo $ap?>&pn=<?php echo $pn?>&php=popuppicsd&plt=1" data-ajax="false" class="ui-btn ui-btn ui-btn-inline ui-corner-all ui-mini ui-btn-icon-left ui-icon-bars"><?php if($_SESSION[tn]==PRC){echo '专案应用页列表';}else if($_SESSION[tn]==EN){echo 'Project Page List';}else{echo '專案應用頁列表';}?></a>
	<?php ;}?>
...
<!--<p><b style="color:PINK"><?php if($_SESSION[tn]==PRC){echo '浏览器';}else if($_SESSION[tn]==EN){echo 'Browser';}else{echo '瀏覽器';}?></b>
	...
	<hr>!-->
	<p><b  style="color:PINK"><?php if(!$roww[pjnbr])$roww[pjnbr]='?';
	if($_SESSION[tn]==PRC){echo '使用';}else if($_SESSION[tn]==EN){echo 'Use';}else{echo '使用';}?></b>
	...
	<p><?php if($_SESSION[tn]==PRC){echo '您应将您设计的文字内容另用软件储存,再复制到格内。';}else if($_SESSION[tn]==EN){echo 'It could be better to save your designed text content as any computer data and copy them to the grid when editing.';}else{echo '您應將您設計的文字內容另用軟件儲存,再複制到格內。';}?></p>
	<hr>
	...
	<p><?php if($_SESSION[tn]==PRC){echo '在上一项巳选用一行格数或在此页点选含+号的格数创建。';}else if($_SESSION[tn]==EN){echo 'You can select a grid number for a row or click the + symbol button to create number of grid per row.';}else{echo '在上一項巳選用一行格數或在此頁點選含+號的格數創建。';}?></p>
	e.g. <i class="fa-plus-circle" style="color:blue"></i> -12 <?php if($_SESSION[tn]==PRC){echo '是指一行含十二平均格,此选项是一格佔一行。';}else if($_SESSION[tn]==EN){echo 'One row contains 12 grids in average width. It is about using one grid [with 12 grid width] for a row.';}else{echo '是指一行含十二平均格,此選項是一格佔一行。';}?>
	<hr>

	<p><b  style="color:PINK"><?php if($_SESSION[tn]==PRC){echo '相片或文字';}else if($_SESSION[tn]==EN){echo 'Picture or text content';}else{echo '相片或文字';}?></b>
	<?php if($_SESSION[tn]==PRC){echo '您能自定行数及格数。按指定方法键入相片档url或键入自定文字内容。';}else if($_SESSION[tn]==EN){echo 'You can define number of rows and grids. You can enter the picture url or your designed textual content by specific methods.';}else{echo '您能自定行數及格數。按指定方法鍵入相片檔url或鍵入自定文字內容。';}?></p>
	<p><?php if($_SESSION[tn]==PRC){echo '在[img=]及[img]之间键入格内的相片url,e.g. picture.gif或http://....。在[imgpop=]及[imgpop]之间键入popup内的相片丶应用页或网页url。不能键入任何其他文字内容。对於相片的修改只能删除再创建。';}else if($_SESSION[tn]==EN){echo 'You can enter picture url between [img=] and [img] for grid picture, e.g. picture.gif or http://.... You can enter url of picture, APP page or web page between [imgpop=] and [imgpop] for popup content. You cannot enter any more content for this situation. Also , you can only delete the picture grid and rebuild and move a new grid.';}else{echo '在[img=]及[img]之間鍵入格內的相片url,e.g. picture.gif或http://....。在[imgpop=]及[imgpop]之間鍵入popup內的相片、應用頁或網頁url。不能鍵入任何其他文字內容。對於相片的修改只能刪除再創建。';}?></p>
	
	<p><?php if($_SESSION[tn]==PRC){echo '在[imgtitle=]及[imgtitle]之间及[imgfooter=]及[imgfooter]之间键入相关标题及备注的词语,您应注意细屏尺寸。';}else if($_SESSION[tn]==EN){echo 'You can enter text content between [imgtitle=] and [imgtitle] and [imgfooter=] and [imgfooter] for picture title and footer. You can enter short word only for mobile design.';}else{echo '在[imgtitle=]及[imgtitle]之間及[imgfooter=]及[imgfooter]之間鍵入相關標題及備註的詞語,您應注意細屏尺寸。';}?></p>
	
	<p><?php if($_SESSION[tn]==PRC){echo '您能在[imgcolor=]及[imgcolor]之间键入rgb颜色码 e.g. rgb(125,125,125), rgba颜色码 e.g. rgba(125,125,125,0.5) 或 hex颜色码 e.g. #123456。是相片框的背景色。';}else if($_SESSION[tn]==EN){echo 'You can enter rgb color code e.g. rgb(125,125,125), rgba color code e.g. rgba(125,125,125,0.5) or hex color code e.g. #123456  between [imgcolor=] and [imgcolor]. It is about the background of picture frame.';}else{echo '您能在[imgcolor=]及[imgcolor]之間鍵入rgb顏色碼 e.g. rgb(125,125,125), rgba顏色碼 e.g. rgba(125,125,125,0.5) 或 hex顏色碼 e.g. #123456。是相片框的背景色。';}?></p>
	 
	<p><?php if($_SESSION[tn]==PRC){echo '若将格用作文字内容,您能将预设内容移除才键入文字或html码。';}else if($_SESSION[tn]==EN){echo 'If you want to enter text content to the grid, you need to remove all the tags. e.g. [imgs=]';}else{echo '若將格用作文字內容,您能將預設內容移除才鍵入文字或html碼。';}?></p>
	<hr>
	<p><b  style="color:PINK"><?php if($_SESSION[tn]==PRC){echo '储存及预览';}else if($_SESSION[tn]==EN){echo 'Save and preview';}else{echo '儲存及預覽';}?></b>
	<i class="fa-save" style="color:PINK"></i><?php if($_SESSION[tn]==PRC){echo '储存按钮';}else if($_SESSION[tn]==EN){echo 'Save button';}else{echo '儲存按鈕';}?>
	...
	<p><?php if($_SESSION[tn]==PRC){echo '在此页的右上方含储存按钮,您只能在页顶的预览内测试您此段的设计。';}else if($_SESSION[tn]==EN){echo 'You can find a save button on the top right corner on this page. You can test this part\'s design by preview function on the top of this page.';}else{echo '在此頁的右上方含儲存按鈕,您只能在頁頂的預覽內測試您此段的設計。';}?></p>
	
	<p><?php if($_SESSION[tn]==PRC){echo '储存完毕是到预览型式,点撀预览按钮到编辑型式。';}else if($_SESSION[tn]==EN){echo 'After saving design content, you can click the preview button to return this page to editing pattern.';}else{echo '儲存完畢是到預覽型式,點撀預覽按鈕到編輯型式。';}?></p>
...
<hr>
	...
	<i class="fa-times" style="color:PINK"></i><?php if($_SESSION[tn]==PRC){echo '刪除按钮';}else if($_SESSION[tn]==EN){echo 'Clear button';}else{echo '刪除按鈕';}?>
...
<?php if($_SESSION[tn]==PRC){echo '您须填写资料,并点撀作实按钮将内容复制到格内,再进行储存。';}else if($_SESSION[tn]==EN){echo 'You fill in the information on the popup form. You need to click the confirm button to copy edited content to the grid. You need to save your design by clicking save button.';}else{echo '您須填寫資料,並點撀作實按鈕將內容複制到格內,再進行儲存。';}?></p>
	<hr>
	...
	<?php if($_SESSION[tn]==PRC){echo 'URL/档名是填有关格内的相片,用户点撀格显示popup。';}else if($_SESSION[tn]==EN){echo 'URL/file name is about the grid content\'picture. APP user clicks the grid to show popup.';}else{echo 'URL/檔名是填有關格內的相片,用戶點撀格顯示popup。';}?></p>
...
<a href="#tpsn"  data-rel="popup" data-transition="pop" class="ui-btn ui-btn-b ui-btn-inline ui-corner-all ui-mini">? <?php if($_SESSION[tn]==PRC){echo '文字內容解释';}else if($_SESSION[tn]==EN){echo 'Text content explanation';}else{echo '文字內容解釋';}?></a>
	...
	<?php if($_SESSION[tn]==PRC){echo '您须使用编辑器填写文字内容,并点撀作实按钮将内容复制到格内,再进行储存。';}else if($_SESSION[tn]==EN){echo 'You can use the web html editor to edit text content. You need to click the confirm button to copy edited content to the grid. You need to save your design by clicking save button.';}else{echo '您須使用編輯器填寫文字內容,並點撀作實按鈕將內容複制到格內,再進行儲存。';}?></p>
	...
	<p><b style="color:PINK"><?php if($_SESSION[tn]==PRC){echo '链结填写';}else if($_SESSION[tn]==EN){echo 'Insert Link';}else{echo '鏈結填寫';}?></b>
...
<a href="popuppicsd.php?pj=<?php echo $roww[pjnbr]?>&ap=<?php echo $roww[ap]?>&g=<?php echo $g?>&pn=<?php echo $pn?>&pres=1&typ=popuppicsd" data-ajax="false" class="ui-btn ui-btn-f ui-btn-inline ui-corner-all ui-mini" style="color:pink" ><?php if($_SESSION[tn]==PRC){echo '改用上次储存';}else if($_SESSION[tn]==EN){echo 'Using previous save';}else{echo '改用上次儲存';}?></a>
<?php ;}else{?>
<a href="popuppicsd.php?pj=<?php echo $roww[pjnbr]?>&ap=<?php echo $roww[ap]?>&g=<?php echo $g?>&pn=<?php echo $pn?>&typ=popuppicsd"  data-ajax="false" class="ui-btn ui-btn-f ui-btn-inline ui-corner-all ui-mini" style="color:pink" ><?php if($_SESSION[tn]==PRC){echo '不改用上次储存';}else if($_SESSION[tn]==EN){echo 'Not using previous save';}else{echo '不改用上次儲存';}?></a>
<?php ;}?>
	<?php if($tl)$tln = '&tl='.$tl;?>
...
<hr>
	...
	<input type="checkbox" name="wsp1" id="imgwsp1" value="1"><label for="imgwsp1"><?php if($_SESSION[tn]==PRC){echo '文字显示';}else if($_SESSION[tn]==EN){echo 'Show all texts ';}else{echo '文字顯示';}?></label>
...
<script> 
    $(document).ready(function(){
		...
		$('#imgsave').click(function(event){
			...
			var imgwsp = ''; if($('#imgwsp1').attr('checked'))imgwsp =1;
			var html=''; var imgtclrhtml='';var imgclrhtml='';var imgwsphtml ='';
			
			if(imgtclr){imgtclrhtml = 'color:'+imgtclr+';';}else{imgtclrhtml ='';}
			if(imgclr){imgclrhtml = 'background-color:'+imgclr+';';}else{imgclrhtml ='';}
			if(imgwsp){imgwsphtml = 'white-space:normal;';}else{imgwsphtml ='';}
			
	
			if(imgtitle){imgtitle = '<p>'+imgtitle+'</p>';}else{imgtitle ='' ;}
			if(imghtml){var br= '';}else{var br='' ;}
			if(imgftr){imgftr= '<p>'+imgftr+'</p>';}else{imgftr='' ;}
			if(img || imgtitle || imgftr){
				if(hrefhtml){var imgspop= '';var dataurl= '';}else{var imgspop= 'imgspop ';hrefhtml= '#';var dataurl= 'data-pop="<?php echo $pj.$ap ?>" data-url="'+imgshtml+'"';}
				html += '<a class="'+imgspop+'imghtml ui-link" href="'+hrefhtml+'"  data-img="'+img+'@#@'+imgtitle+'@#@'+imgftr+'@#@'+imgs+'@#@'+imgtclr+'@#@'+imgclr+'@#@'+imgwsp+'" style="text-decoration:none;padding:1px;'+imgwsphtml+imgtclrhtml+imgclrhtml+'" '+dataurl+'>'+imgtitle;
				if(imghtml)html += '<img style="width:100%" src="'+imghtml+'">';
				html += imgftr+'</a>';
			}else{html +='' ;}
		...
		$(document).on('click', '.imghtml',function (event) {		event.preventDefault();		
			...
			if(imghtm[6])$('#imgwsp1').prop("checked", true);
...
if($roww[theme]){$theme = $roww[theme];}else{$theme = '';}
	...
	echo "<meta http-equiv='refresh' content='0;URL=popuppicsd.php?ap=".htmlspecialchars($roww[ap])."&pj=".htmlspecialchars($roww[pjnbr])."&pn=".htmlspecialchars($pn)."'>";
;}
